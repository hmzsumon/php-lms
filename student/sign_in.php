...
if(isset($_POST['login'])) {
    ...
    $result= mysqli_query($db_conn, " SELECT * FROM `students` WHERE `email` = '$email' OR `username` = '$email' ");
    if(mysqli_num_rows($result) === 1) {
        ...
        if(password_verify($password, $row['password'])) {
            if($row['status'] == 1) {
                ...
                $loging_err = "Your account inactive";
            }
        } else {
            ...
            <form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST">
              <div class="form-group mt-md">
                <span class="input-with-icon">
                  <input type="text" class="form-control" name="email" placeholder="Email" value="student@gmail.com">
